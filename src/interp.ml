...
let is_subclass : string -> string -> class_env -> exp_val ea_result = 
  ...
  let rec is_subclass' : string -> string -> class_env -> exp_val ea_result = 
    fun c1 c2 ce -> 
      (match ce with
      | [] -> error "is_subclass: class c2 not found"
      | (c1,(c,_,_))::t when c == c2 -> return @@ BoolVal (true)
      | (c1,(c,_,_))::t -> return @@ BoolVal (false)
      | (_,(_,_,_))::t -> is_subclass' c1 c2 t)
  in is_subclass' c_name1 c_name2 c_env
...
and
  ...
  | IsInstanceOf(e,id) ->
    eval_expr e >>=
    obj_of_objectVal >>= fun o ->
    (match o with
    | (c_name,_) -> (is_subclass c_name id !g_class_env))
  | Cast(e,id) ->
    eval_expr e >>= fun ev ->
    return ev
  (* Debug *)
...
let read_file (filename:string) : string = 
  ...
  try
    while true do
      lines := (input_line chan^" ") :: !lines (* the space acts as line separator *)
    done;
